            ...
            // return $datas[0];
             ...
             foreach($datas[0] as $row)
             {
                 ...
                 if (!$old_user) 
                 {
                     ...
                     $user=User::create(['first_name'=>$row['firstname'],'last_name'=>$row['lastname'],'email'=>$row['email'],'phone'=>$row['phone'],'emp_num'=>$row['staffid'],'sex'=>$gender, 'address'=>$row['address'],'marital_status'=>$row['marital_status'],'dob'=>$row['date_of_birth'],'branch_id'=>($branch ? $branch->id : 0),
...
//        return back();
    ...
    public function importDependants(Request $request)
    {
        ...
        if($request->hasFile('template')){
            ...
            // when id not present replace with 0//let client fill out later

            foreach($datas[0] as $row){
                $dependant=Dependant::where('name',$row['name'])->where('user_id',$row['employee_id'])->first();
                if (!$dependant) {
                    Dependant::create(['name'=>$row['name'] ?? 'nil','user_id'=>$row['employee_id'] ?? 0,'email'=>$row['email'],'phone'=>$row['phone'],
                        'dob'=>$row['date_of_birth'] ?? '9/1/1964','relationship'=>$row['relationship'] ?? 'nil','company_id'=>$company->id]);
                }
            }
